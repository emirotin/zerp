...
function rewriteRelativeUrls(html, relativeSlidePath) {
    ...
    return html.replace(/\b(src|href|poster)\s*=\s*(["'])([^"']+)\2/gi, (_match, attr, quote, value) => {
        ...
        const rewritten = path.posix.normalize(normalizedDir ? path.posix.join(normalizedDir, value) : value);
...
export async function buildPresentationHtml(options) {
    ...
    const slideHtmlParts = await Promise.all(slideFiles.map(async ({ absolutePath, relativePath }) => {
