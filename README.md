...
- `zerp check` also warns about characters no bundled font can draw. A built deck carries inlined Montserrat and Roboto Mono subsets plus a one-glyph face for `→`; anything outside them (`≈`, `▲`, CJK, …) is drawn by whatever the viewing machine falls back to, which differs per OS and again on export. The warning lists the characters and their codepoints. Emoji are exempt — every platform draws those from its own colour emoji font.
